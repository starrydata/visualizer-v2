...
def test_get_graph_by_property_response_format(mock_get, mock_response):
    ...
    assert [p.x for p in data_points_list[0].data_points] == [299.8597, 324.8683, 349.8757]
    assert [p.y for p in data_points_list[0].data_points] == [-148.4452, -160.2763, -172.9511]
    assert [p.x for p in data_points_list[1].data_points] == [1, 2]
    assert [p.y for p in data_points_list[1].data_points] == [10, 20]
...
def test_get_graph_by_property_not_found(mock_get, mock_response):
    ...
    from infra.graph_repository import GraphRepositoryApiCleansingDataset
    repo = GraphRepositoryApiCleansingDataset()
    # 存在しないpropertyでも空リストが返る想定
    data_points_list = repo.get_graph_by_property("NotExist", "Seebeck coefficient")
    assert isinstance(data_points_list, list)
