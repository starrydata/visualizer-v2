...
def test_get_graph_by_property_response_format(mock_get, mock_response):
    ...
    assert [p.x for p in data_points_series.data[0].data] == [299.8597, 324.8683, 349.8757]
    assert [p.y for p in data_points_series.data[0].data] == [-148.4452, -160.2763, -172.9511]
    assert [p.x for p in data_points_series.data[1].data] == [1, 2]
    assert [p.y for p in data_points_series.data[1].data] == [10, 20]
...
def test_get_graph_by_property_not_found(mock_get, mock_response):
    ...
    from infra.graph_repository import GraphRepositoryApiCleansingDataset
    repo = GraphRepositoryApiCleansingDataset()
    data_points_series = repo.get_graph_by_property("NotExist", "Seebeck coefficient")
    assert hasattr(data_points_series, "data")
    assert isinstance(data_points_series.data, list)
